...
Future<void> runStoreMetadataAudit() async {
  ...
  final androidPath = 'android/fastlane/metadata/android';
  final iosPath = 'ios/fastlane/metadata';

  int issues = 0;
  final List<String> findings = [];

  await loadingSpinner('Auditing local store metadata and assets', () async {
    // 1. Check for basic metadata structure
    if (!Directory('fastlane').existsSync()) {
      findings.add(
    ...
    if (Directory(iosPath).existsSync()) {
      final languages = Directory(iosPath).listSync().whereType<Directory>();
      for (final lang in languages) {
        final langCode = p.basename(lang.path);
