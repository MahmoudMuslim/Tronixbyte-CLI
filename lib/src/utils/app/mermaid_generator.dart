...
Future<void> generateProjectDashboard() async {
  ...
  await loadingSpinner(
    'Generating Elite Project Dashboard (Mermaid Diagram) in $activePath',
    () async {
      ...
      if (libDir.existsSync()) {
        ...
        buffer.writeln(
          '| Avg. Lines per File | ${(totalFiles > 0 ? totalLines / totalFiles : 0).toStringAsFixed(1)} |',
        );
      }
      ...
      buffer.writeln('| :--- | :--- |');
      buffer.writeln('| `tools sync` | Dependency & Code-gen Sync |');
      buffer.writeln('| `tools repair` | Nuclear Infrastructure Fix |');
      buffer.writeln('| `tools doctor` | Architecture & Health Check |');

      buffer.writeln('\n---');
      buffer.writeln('*Generated by ⚡ Tronixbyte CLI*');

      final outFile = File(p.join(activePath, 'STRUCTURE.md'));
      outFile.writeAsStringSync(buffer.toString());
    },
  );

  printSuccess('Generated: STRUCTURE.md at the project root.');
}
