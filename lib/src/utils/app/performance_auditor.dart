...
Future<void> runPerformanceAudit() async {
  ...
  await loadingSpinner(
    'Scanning assets and source code for bottlenecks in $activePath',
    () async {
      // 1. Large Image Audit
      ...
      if (assetsDir.existsSync()) {
        ...
        for (final image in images) {
          ...
          if (size > 1024 * 1024) {
            // > 1MB
            details.add(
              '$red$bold[Size]$reset Large image: ${p.relative(image.path, from: activePath)} (${(size / (1024 * 1024)).toStringAsFixed(2)} MB)',
            );
            issues++;
          }
        }
      ...
      // 3. Font Size Check
      final fontsDir = Directory(p.join(activePath, 'assets', 'fonts'));
      if (fontsDir.existsSync()) {
        ...
        for (final font in fonts) {
          if (font.lengthSync() > 500 * 1024) {
            // > 500KB
            details.add(
              '$cyan$bold[Font]$reset Heavy font file: ${p.relative(font.path, from: activePath)} (${(font.lengthSync() / 1024).toStringAsFixed(0)} KB)',
            );
          }
        }
      }
